    ...
    return (
        <div className='flex fixed top-0 w-full items-start justify-between z-4'>
            ...
            onClick={() => setNavOpen(true)}
                ...
                className='bg-black lg:h-14 h-14 lg:w-[17vw] w-60 relative'>
                <div ref={navGreenRef} className='bg-[#d3d550] transition-all  w-full absolute top-0 h-0'></div>
                <div className='relative h-full w-full lg:px-12 px-8 flex flex-col justify-center items-end gap-1'>
                    <div className='h-0.5 w-12 bg-white rounded-full'></div>
                    <div className='h-0.5 w-8 bg-white rounded-full'></div>
                </div>
            </div>
        </div>
    )
